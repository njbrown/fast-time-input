...
function parseDateAsText(timeString: string, format?: "24hr" | "12hr") {
    let newTime = "";
    ...
    if (/\S/.test(timeString) || timeString !== "") {
        const t = convertInput(timeString);
...
*/
function convertInput(timeString: string) {
    var meridian = findMeridian(timeString);

    // strip extra characters
    timeString = timeString.replace(/\D/g, "");

    if (timeString.length == 0) return new Time(0, 0);
    if (timeString.length == 1 || timeString.length == 2) return convertSingleAndDouble(timeString, meridian);
    if (timeString.length == 3) return convertTriple(timeString, meridian);
    if (timeString.length == 4) return convertQuadruple(timeString, meridian);

    return new Time(0, 0);
}

// returns am, pm or an empty string if neither is found
function findMeridian(timeString: string): "am" | "pm" | null {
...
function convertSingleAndDouble(timeString: string, meridian: "am" | "pm" | null) {
...
*/
function convertTriple(timeString: string, meridian: "am" | "pm" | null) {
...
function convertQuadruple(timeString: string, meridian: "am" | "pm" | null) {
...
export function parse(timeString: string, format?: "24hr" | "12hr"): string {
    return parseDateAsText(timeString, format);
}
